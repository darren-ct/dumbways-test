...
import { GET_PRODUCTIVITIES} from "../../lib/names/productivity"
...
import ProductivityRow from "../../components/rows/ProductivityRow";
import DeleteProductivity from "../../components/modals/DeleteProductivity"
import EditProductivity from "../../components/modals/EditProductivity"
import PostProductivity from "../../components/modals/PostProductivity"

export const ProductivityContext = createContext(null)


const Productivity = (props) => {
  const {push} = useRouter();
  const {loading,error,data, refetch} = useQuery(GET_PRODUCTIVITIES, {notifyOnNetworkStatusChange:true,variables:{
    where: {
      student: {
        classesId: props.id
      }
  ...
  return (
  <ProductivityContext.Provider value={{refetch, setSuccessMsg, setOnPost, setOnEdit, setDeleteModal ,setOnDelete, setEditId, classId:props.id}}>
      <div className="relative xl:container xl:mx-auto px-8 py-4 flex flex-col items-center">
           {deleteModal && <DeleteProductivity setDeleteModal={setDeleteModal} id={onDelete}/> }
           {onEdit && <EditProductivity id={editId} setOnEdit={setOnEdit}/>}
           {onPost && <PostProductivity setOnPost={setOnPost} ids={data.productivities.map(productivity => productivity.student.id)}/>}
          ...
          <table className="relative w-full max-w-2xl border rounded-3xl border-slate-200 mt-4 mb-12">
              ...
              <tbody>
                  {data.productivities.map((productivity,index) => <ProductivityRow item={productivity} key={productivity.id} index={index}/>)}
              </tbody> 
         </table>
      </div>
